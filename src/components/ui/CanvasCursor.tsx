...
export default function CanvasCursor() {
  ...
  const baseHue = 280; // Violet/purple — no blue
  ...
  useEffect(() => {
    ...
    const createParticle = (x: number, y: number) => {
      ...
      // Cycle through violet → fuchsia → rose → amber to avoid blue
      const hue = baseHue + (Math.random() * 120 - 60);
      ...
      for (let i = 0; i < particles.current.length; i++) {
        ...
        if (isDark) {
          ...
          gradient.addColorStop(
            0,
            `hsla(${p.hue}, 85%, 75%, ${currentOpacity})`,
          );
          gradient.addColorStop(
            0.5,
            `hsla(${p.hue}, 75%, 65%, ${currentOpacity * 0.3})`,
          );
          ...
          gradient.addColorStop(
            0,
            `hsla(${p.hue}, 90%, 50%, ${currentOpacity})`,
          );
          gradient.addColorStop(
            0.4,
            `hsla(${p.hue}, 85%, 60%, ${currentOpacity * 0.5})`,
          );
        ...
        for (
          let j = i + 1;
          j < Math.min(i + 3, particles.current.length);
          j++
        ) {
